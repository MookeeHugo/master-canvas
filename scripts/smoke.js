...
let appPort = Number(process.env.MASTER_CANVAS_SMOKE_APP_PORT || 5173);
let appUrl = process.env.MASTER_CANVAS_SMOKE_URL || `http://127.0.0.1:${appPort}`;
const appUrlFromEnv = Boolean(process.env.MASTER_CANVAS_SMOKE_URL);
...
function devServerArgs() {
  return process.platform === 'win32'
    ? ['/d', '/s', '/c', `npm run dev -- --host 127.0.0.1 --port ${appPort} --strictPort`]
...
function isMasterCanvasHtml(response) {
  return response?.statusCode &&
    response.statusCode < 500 &&
    response.body.includes('<div id="app"') &&
    response.body.includes('Master Canvas');
}

async function chooseAvailableAppPort() {
  if (appUrlFromEnv) return;
  for (let candidate = appPort + 1; candidate < appPort + 30; candidate += 1) {
    const candidateUrl = `http://127.0.0.1:${candidate}`;
    try {
      const response = await getText(candidateUrl, 300);
      if (isMasterCanvasHtml(response)) {
        appPort = candidate;
        appUrl = candidateUrl;
        return;
      }
  ...
  throw new Error('No free local port found for Master Canvas smoke server');
}

async function waitForMasterCanvasHttp(url, label) {
  const deadline = Date.now() + 20000;
  let lastError;
  while (Date.now() < deadline) {
    try {
      const response = await getText(url);
      if (isMasterCanvasHtml(response)) return response;
      lastError = new Error(`${label} returned a non-Master Canvas page on ${url}`);
    } catch (err) {
      lastError = err;
    }
    await sleep(300);
...
async function ensureDevServer() {
  try {
    const response = await getText(appUrl, 800);
    if (isMasterCanvasHtml(response)) return null;
    await chooseAvailableAppPort();
  } catch {
    // Start a local Vite server below.
  }

  const child = spawn(npmCommand, devServerArgs(), {
    cwd: process.cwd(),
    stdio: 'ignore',
    windowsHide: true,
  });
  await waitForMasterCanvasHttp(appUrl, 'Master Canvas dev server');
  return child;
}
...
async function waitForAppReady(ws) {
  ...
  while (Date.now() < deadline) {
    ...
    if (lastState.hasApp && lastState.readyState !== 'loading') return;
    await sleep(250);
  }
  throw new Error(`Master Canvas app shell did not render: ${JSON.stringify(lastState)}`);
}
...
async function runBrowserCheck() {
  ...
  try {
    ...
    const state = await evaluate(ws, `(async () => {
      ...
      return {
        ...
        topbar: (() => {
          ...
          return {
            appRows: appEl ? getComputedStyle(appEl).gridTemplateRows : '',
    ...
    console.log(JSON.stringify({ appUrl, screenshotPath, ...state }, null, 2));
